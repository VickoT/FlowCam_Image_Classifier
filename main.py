...
class FlowCamReportGenerator:
    ...
    # '_' is used to indicate that this method is private, and only used locally
...
class FlowCamProcessor:
    ...
    def image_separator(self):
        try:
            os.chdir(self.paths.raw_dir)
        except FileNotFoundError:
            print(f'Error: {self.paths.raw_dir} not found')
            return

        for filename in [f for f in os.listdir(self.paths.raw_dir) if f.endswith(".lst")]:
            sample_name = os.path.splitext(filename)[0]
            sample_outpath = self.paths.extract_dir
            fp = os.path.join(self.paths.raw_dir, filename)
            ...
            for id in meta["id"]:
                ...
                cp = os.path.join(self.paths.raw_dir, collage_filename)
    ...
    def run_classifier(self):
        csv_file = os.path.join(self.paths.raw_dir,
                                f"{os.path.basename(self.paths.raw_dir)}.csv")
        df = pd.read_csv(csv_file)
        model = joblib.load(self.paths.model_path)
        features = ['Area (Filled)', 'Aspect Ratio', 'Circle Fit',
                    'Circularity', 'Circularity (Hu)', 'Compactness',
                    'Diameter (FD)', 'Edge Gradient', 'Elongation',
                    'Geodesic Aspect Ratio', 'Geodesic Thickness', 'Intensity',
                    'Ratio Red/Blue', 'Roughness', 'Transparency']
        ...
        for file in os.listdir(self.paths.extract_dir):
            path_file = os.path.join(self.paths.extract_dir, file)
